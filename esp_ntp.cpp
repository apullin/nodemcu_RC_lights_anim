...
void getCurrTime(clockTime* ct) {
    ct->hours = lastNTP.hours;
    ct->minutes = lastNTP.minutes;
    ct->seconds = lastNTP.seconds;
    ct->updateTimeMillis = lastNTP.updateTimeMillis;
}

unsigned long ct_to_day_seconds(clockTime ct) {
    unsigned long secs = ct.hours * 3600 + ct.minutes * 60 + ct.seconds;
    return secs;
}
